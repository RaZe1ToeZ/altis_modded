...
private _vaultHouse = [[["Altis", "Land_BackAlley_01_l_gate_F"], ["Tanoa", "Land_Medevac_house_V1_F"]]] call TON_fnc_terrainSort;
...
if (((nearestObject [_pos,"Land_MilOffices_V1_F"]) == _building || (nearestObject [_pos,_vaultHouse]) == _building) && (west countSide playableUnits < (LIFE_SETTINGS(getNumber,"minimum_cops")))) exitWith {
    hint format [localize "STR_Civ_NotEnoughCops",(LIFE_SETTINGS(getNumber,"minimum_cops"))];
};
if ((typeOf _building) == _vaultHouse && (nearestObject [_pos,"Land_MilOffices_V1_F"]) getVariable ["locked",true]) exitWith {hint localize "STR_ISTR_Bolt_Exploit"};
...
if ((nearestObject [_pos,"Land_MilOffices_V1_F"]) == _building || (nearestObject [_pos,_vaultHouse]) == _building) then {
